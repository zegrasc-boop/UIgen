...
vi.mock("../ToolInvocationDisplay", () => ({
  ToolInvocationDisplay: ({ toolName, args }: { toolName: string; args?: Record<string, unknown> }) => (
    <div data-testid="tool-invocation">{args?.command === "create" ? `Created ${args?.path}` : toolName}</div>
  ),
}));
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByTestId("tool-invocation")).toBeDefined();
});
